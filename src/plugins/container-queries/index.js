...
export function createContainerQueriesPlugin({canUseDOM}) {
  let Plugin = {
    reserve(key) {
  ...
  if (canUseDOM) {
    Plugin.resolve = function(rules, {stylishState}) {
      let matchingQueries = stylishState.containerQueries || [];
      return matchingQueries.map((query) => rules[query]);
    };

    Plugin.decorate = function(Component, {stylesheet, React}) {
...
export default createContainerQueriesPlugin(environment);
